...
const PostView = ({
  ...
  return (
    <div className='flex flex-col w-full mb-10 bg-white rounded-lg p-10 shadow border text-black shadow-xl'>
        <h1 className='text-4xl font-bold capitalize text-black mb-5'>
            {
                postData.title
            }
        </h1>
        <div className='flex flex-col gap-3 text-l'>
            ...
            <div>
                <strong>Published:</strong> { formatDistance(postData?.author?.created_at, new Date(), { addSuffix: true }) }
            </div>
            <div>
                <strong>Updated:</strong> { formatDistance(postData?.author?.updated_at, new Date(), { addSuffix: true }) }
            </div>
        </div>
